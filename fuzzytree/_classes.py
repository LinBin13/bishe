...
class BaseFuzzyDecisionTree(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None, check_input=True):
        ...
        if is_classifier(self):
            if hasattr(self, "fuzzy_sets") and self.fuzzy_sets is not None:
                self.tree_ = FuzzyTree(self.y_, sample_weight, fuzzy_sets=self.fuzzy_sets,
...
class FuzzyDecisionTreeClassifier(ClassifierMixin, BaseFuzzyDecisionTree):
    ...
    def __init__(
        ...
        self.fuzzy_sets = fuzzy_sets  #new 保存自定义模糊集
        self.feature_names = feature_names  #new 保存特征名
